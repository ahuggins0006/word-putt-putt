...
* [`src/project_lish/styles.cljs`](src/project_lish/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/project_lish/`](src/project_lish/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/project_lish/core.cljs): contains the SPA entry point, `init`
* [`test/project_lish/`](test/project_lish/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `project_lish` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/project_lish/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns project-lish.example
  (:require [project-lish.config :as config])
